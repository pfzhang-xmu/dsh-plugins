...
function followUpSender() {
  ...
  if (IS_DSH_MODE) {
    // DSH mode: post to the parent GUI; the dsh-cowart client plugin forwards
    // it to the agent conversation as a [cowart-request:<type>] user message.
    sendMessage = (message, analyticsContext = {}) => {
      try {
        window.parent?.postMessage(
          { source: 'dsh-cowart', type: 'agent-request', payload: { message, analyticsContext } },
          '*'
        )
      } catch (error) {
        console.error('Cowart DSH bridge send failed', error)
      }
      return Promise.resolve()
    }
  } else if (typeof window.cowartMcp?.sendFollowUpMessage === 'function') {
...
export default function App() {
  ...
  const handleMount = useCallback((editor) => {
    ...
    if (hasCowartWidgetBridge()) {
      if (IS_DSH_MODE && 'EventSource' in window) {
        canvasEvents = new window.EventSource(cowartCanvasEventsUrl())
